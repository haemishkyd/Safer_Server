...
def build_response(connection,cursor):
...
def build_login_response(passedusername,connection,cursor):
    #We need these to be the global ones since these have been set up to access the database
    db = connection
    cur = cursor
    #please see calls_query.sql for this query in a better format
...
def hello(request):
    ...
    if 'userdata' in request.GET:       
        data = request.GET.get('userdata','No Data')
        if (len(str(data))>10):
            ...
            if (message_type == 'user_data'):
                ...
                response = build_response(db,cur)                    
                logdiagdata("--User Response Complete")  
    if 'agentdata' in request.GET:
        data = request.GET.get('agentdata','No Data')
        if (len(str(data))>10):
            e=ET.fromstring(str(data))
            message_type = e.tag
    ...
    if 'logindata' in request.GET:
        data = request.GET.get('logindata','No Data')
        if (len(str(data))>10):      
            e=ET.fromstring(str(data))          
            message_type = e.tag
            if (message_type == 'login_data'):
                ...
                if (record_exists != 0):
                    ...
                    response = build_login_response(username,db,cur)
                else:
    ...
    if 'getagentdata' in request.GET:
        data = request.GET.get('getagentdata','No Data')
        if (len(str(data))>10):
            e=ET.fromstring(str(data))
            message_type = e.tag
